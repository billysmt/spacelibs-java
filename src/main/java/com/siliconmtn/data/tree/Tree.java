...
public class Tree implements Serializable {
    ...
    public Tree(List<Node> data, Node root) {
        ...
        // need to use a new list here or the passed list is compromised (build removes items from
        // the list). -JM- 11.03.2017
    ...
    private void build(List<Node> data, List<Node> children) {
        ...
        for (Node dataNode : data) {
            for (Node childNode : children) {
                if (StringUtil.defaultString(dataNode.getParentId())
                        .equalsIgnoreCase(childNode.getNodeId())) {
    ...
    public void buildNodePaths(Node parentNode, String delimiter, boolean useName) {
        ...
        for (Node child : parentNode.getChildren()) {
            ...
            path.append(
                    useName && !StringUtil.isEmpty(child.getNodeName())
                            ? child.getNodeName()
                            : child.getNodeId());
