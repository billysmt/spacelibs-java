...
public class NumberUtil {
    ...
    public static int toInt(String val, int defaultVal, boolean removeNonAlphaNum) {
        ...
        Long l =
                Long.valueOf(
                        processLongInt(
                                val, checkForNegativeValue(val), removeNonAlphaNum, defaultVal));
     ...
     */
    private static long processLongInt(
            String val, boolean negative, boolean removeNonAlphaNum, long defaultValue) {

        long convertedString = 0;

        try {
            // Only remove alpha characters if requested.  replace the negative sign that was
            // removed by remove non alpha
            if (removeNonAlphaNum) {
                convertedString =
                        Long.parseLong(
                                signedValue(StringUtil.removeNonAlphaNumeric(val), negative));
            } else {
    ...
    public static float toFloat(String val, float defaultValue, boolean removeNonAlphaNum) {
        ...
        return processFloat(
                val.trim(), checkForNegativeValue(val), removeNonAlphaNum, defaultValue);
    }
     ...
     * @return
     */
    private static float processFloat(
            String value, boolean negative, boolean removeNonAlphaNum, float defaultValue) {
        try {
     ...
     * @return Double value from meta data
     */
    private static double processDouble(
            String value, boolean negative, boolean removeNonAlphaNum, double defaultValue) {
        try {
    ...
    private static String formatValue(String value, boolean negative, boolean removeNonAlphaNum) {
        if (removeNonAlphaNum) {
            ...
            // if there is a decimal, split on the first decimal, remove alphas and put back
            // together
            if (index > -1) {
                String major = "";
                String minor = "";
                ...
                if (index + 1 < value.length())
                    minor = StringUtil.removeNonAlphaNumeric(value.substring(index + 1));
