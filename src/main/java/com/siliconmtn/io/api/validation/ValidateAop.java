...
public class ValidateAop {
     ...
     */
    @Before(
            "@annotation(com.siliconmtn.io.api.validation.Validate) && args(.., @PathVariable"
                    + " pathVar, @RequestBody body)")
    public void beforeAdvice(JoinPoint pjp, Object body, Object pathVar)
        ...
        if (validate != null) {
            ...
            List<ValidationErrorDTO> errors =
                    validateReponse(
                            body, attributes, m.getDeclaringClass().getName() + "." + m.getName());
            if (!errors.isEmpty()) {
     ...
     */
    private List<ValidationErrorDTO> validateReponse(
            Object body, Map<AttributeKey, Object> attributes, String key)
            throws EndpointRequestException {
        ...
        try {
            ...
            throw new EndpointRequestException(
                    "Data validation preperation failed.", e, HttpStatus.INTERNAL_SERVER_ERROR);
        }
